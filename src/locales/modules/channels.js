...
export default {
  ...
  weixinRestart: _("重启 Gateway", "Restart Gateway", "重新啟動 Gateway"),
  weixinRestartHint: _("使插件和登录生效；会短暂中断现有渠道连接", "Apply plugin and login changes; existing channel connections will briefly disconnect", "使外掛與登入生效；將短暫中斷現有渠道連線"),
  weixinLoginSaved: _("扫码登录完成；请重启 Gateway 并检查渠道运行状态", "QR login completed; restart Gateway and check channel runtime status", "掃碼登入完成；請重新啟動 Gateway 並檢查渠道執行狀態"),
  weixinRecommended: _("兼容安装目标：{version}（OpenClaw {host}）", "Compatible install target: {version} (OpenClaw {host})", "相容安裝目標：{version}（OpenClaw {host}）"),
  weixinPublished: _("npm 最新发布：{version}（不自动跨版本线升级）", "Latest on npm: {version} (no automatic upgrade across compatibility lines)", "npm 最新發布：{version}（不自動跨版本線升級）"),
  weixinCompatUnknown: _("兼容性待核实", "Compatibility not yet verified", "相容性待核實"),
  weixinLabel: _('微信', 'WeChat'),
  weixinDesc: _("通过腾讯微信插件接入私信和媒体（不含群聊）", "Direct messages and media via Tencent Weixin (no group chats)", "透過騰訊微信外掛接入私訊與媒體（不含群聊）"),
  weixinGuide1: _("使用腾讯维护的 <strong>@tencent-weixin/openclaw-weixin</strong> 插件，目前仅支持私信和媒体", "Uses Tencent’s <strong>@tencent-weixin/openclaw-weixin</strong> plugin for direct messages and media only", "使用騰訊維護的 <strong>@tencent-weixin/openclaw-weixin</strong> 外掛，目前僅支援私訊與媒體"),
  weixinGuide2: _("一键安装会按当前 OpenClaw 版本选择已核实的兼容插件版本", "Install selects a reviewed plugin version compatible with your OpenClaw version", "一鍵安裝會依目前 OpenClaw 版本選擇已核實的相容外掛版本"),
  ...
  weixinGuide4: _("安装或登录后点击「重启 Gateway」，再检查渠道运行状态；扫码成功不等于消息服务已就绪", "After installation or login, restart Gateway and check channel status; QR login alone does not mean messaging is ready", "安裝或登入後點擊「重新啟動 Gateway」，再檢查渠道執行狀態；掃碼成功不代表訊息服務已就緒"),
  weixinGuide5: _("多账号可重复扫码添加；建议按账号、渠道和发送者隔离会话（session.dmScope = per-account-channel-peer），面板不会自动改动全局会话策略", "Scan again for another account. Consider session.dmScope = per-account-channel-peer for account isolation; the panel does not change global session policy automatically", "多帳號可重複掃碼新增；建議依帳號、渠道及傳送者隔離工作階段（session.dmScope = per-account-channel-peer），面板不會自動變更全域策略"),
  ...
  weixinLoginHint: _("二维码由服务器上的插件生成，凭据保留在服务器本地", "The server-side plugin generates the QR code and stores credentials on that server", "二維碼由伺服器上的外掛產生，憑證保留於伺服器本機"),
